...
public class PlayerGrid : MonoBehaviour
{
    ...
    private void GenerateInitialComponents()
    {
        ...
        GameObject linea = (GameObject)Instantiate(Resources.Load("Line"), gameContainer.transform);
        LineRenderer lineaR = linea.GetComponent<LineRenderer>;

        int col1 = 0;
        int row1 = 1;

        ComponentGenerator.matrix[col1, row1].GetComponent<Transform>();

        lineaR.SetPosition(0, t1.transform.position);
        lineaR.SetPosition(1, t2.transform.position);
    }
    */
